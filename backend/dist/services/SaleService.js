...
export class SaleService {
    static async processSale(saleData, userId) {
        ...
        try {
            const { items, customerId, customerName, // For walk-in customers
            customerPhone, paymentMethod, amountPaid, discountTotal } = saleData;
            ...
            if (!targetCustomerId && customerPhone) {
                ...
                if (existingCustomer) {
                    ...
                    sale.customer = targetCustomerId; // Cast for mongoose id
            ...
            }, { upsert: true, session }).session(session); // Ensure session is used
